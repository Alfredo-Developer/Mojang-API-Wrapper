...
import dev.apposed.apiwrapper.requests.json.inbound.name.status.NameStatus;
...
public class NameStatusRequest {
     ...
     */
    public NameStatusRequest(String name, String bearerToken){
        this.response = new APIRequest(MojangConstants.NAME_STATUS).addPathChange("<nameToCheck>", name).addHeader("Authorization", bearerToken).build();
     ...
     */
    public NameStatus getAsObject(){
        return new Gson().fromJson(this.jsonResponse, NameStatus.class);
    }
